...
var EMOJI_TYPES = require('cz-emoji/lib/types.json');
...
exports.markdown = function(version, commits, options) {
  ...
  return Bluebird.resolve(commits)
    ...
    .each(function(commit) {
      var type;
      if (commit.type.indexOf(':') === 0) {
        EMOJI_TYPES.forEach(function(t) {
          if (t.code === commit.type) {
            type = t.name;
          }
        });
      } else {
        type = TYPES[commit.type] ? commit.type : DEFAULT_TYPE;
      }
    ...
    .each(function(type) {
      ...
      EMOJI_TYPES.forEach(function(t) {
        if (t.name === type) {
          found = true;
          content.push('##### ' + t.emoji + ' ' + t.description);
          content.push('');
        }
      });
